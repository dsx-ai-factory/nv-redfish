...
use crate::oem::dell::job_service::{DellJobService, DellJobServiceSchema};
...
impl<B: Bmc> DellManager<B> {
    ...
    /// Get the legacy Dell OEM jobs collection for compatibility fallback.
    ///
    /// Prefer [`crate::job_service::JobService::jobs`] whenever available.
    #[must_use]
    pub fn legacy_jobs(&self) -> Option<DellJobs<B>> {
        self.links
            .jobs
            .as_ref()
            .map(|jobs| DellJobs::from_legacy_nav(&self.bmc, jobs))
    }
}
