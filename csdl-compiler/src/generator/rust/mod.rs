...
impl<'a> RustGenerator<'a> {
    ...
    pub fn new(compiled: &'a Compiled<'a>, config: Config) -> Result<Self, Error<'a>> {
        ...
        let root = compiled
            ...
            .try_fold(root, |m, (name, t)| {
                let actions = compiled.actions.get(name);
                let forced = ForcedUpdate::new(forced_updates.contains(name));
                m.add_complex_type(t, actions, forced, create_properties.remove(name), &config)
            })?;
        let root = compiled.entity_types.values().try_fold(root, |m, t| {
            let create = create_properties.remove(&t.name);
            let forced = ForcedUpdate::new(forced_updates.contains(&t.name));
            let type_excerpt_copies = compiled
                .excerpt_copies
                .get(&t.name)
                .map(|v| v.iter().collect::<Vec<_>>())
                .unwrap_or_default();
            m.add_entity_type(t, create, type_excerpt_copies, forced, &config)
        })?;
        let root = compiled
            .type_definitions
            .values()
    ...
    /// Each entry contains the ancestor properties followed by the type's own properties.
    /// Navigation properties serialize references, so their targets need no create shape.
    fn create_properties(
        compiled: &'a Compiled<'a>,
    ) -> HashMap<QualifiedName<'a>, Vec<&'a Properties<'a>>> {
        ...
        while let Some(name) = pending.pop() {
            ...
            while let Some(name) = next {
                let (base, own_properties) = if let Some(t) = compiled.complex_types.get(&name) {
                    (t.base, &t.properties)
                ...
                pending.extend(own_properties.properties.iter().filter_map(|p| {
                    let (info, name) = p.ptype.inner();
                    (info.class == TypeClass::ComplexType).then_some(*name)
                }));
                properties.push(own_properties);
                next = base;
            }
            properties.reverse();
            result.insert(name, properties);
        }
        result
    }
